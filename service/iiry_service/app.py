...
def load_sessions() -> dict[str, Session]:
    ...
    try:
        ...
        allowed = {field.name for field in fields(Session)}
        return {state: Session(**{key: value for key, value in item.items() if key in allowed}) for state, item in payload.items()}
    except Exception:
        return {}
